...
wkdir='/Volumes/My Book for Mac/rnaseq_l1_timecourse/rnaseq'
setwd(wkdir)
name=read.table('name.txt',sep='\t',stringsAsFactors = F)
count_table=read.table(paste('count/',name[1,1],'_count.txt',sep=''),stringsAsFactors = F,sep='\t')
...
for (i in 2:72)
{
  tmp=read.table(paste('count/',name[i,1],'_count.txt',sep=''),stringsAsFactors = F,sep='\t')
...
write.table(count_table,'count/htseq_count_merged.txt',sep='\t',col.names = T,row.names = F,quote=F)
